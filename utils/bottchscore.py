...
class BottchScore:
    ...
    def _initialize_mol(self, mol, disable_mesomeric):
        ...
        if self.debug:
            print("DEBUG> SMILES: %s" % self._smiles)
        ...
        for patt, idx_info in self._mesomery_patterns.items():
            ...
            for f in found:
                for pair in idx_pairs:
                    for idx in pair:
                        if self.debug:
                            print("DEBUG> Assigning mesomery:", f, "->", f[idx])
                        self._mesomery_equivalence[f[idx]] = contribution
                    p0 = f[pair[0]]
                    p1 = f[pair[1]]
                    if self.debug:
                        print("DEBUG> Updating automorphs:", p0, p1)
    ...
    def print_table(self):
        ...
        atom_symbols = [
            self.mol.GetAtom(x).GetAtomicNum() for x in self._equivalents.keys()
        ]
    ...
    def print_complexity_calculations(self):
        ...
        for idx in list(self._indices.keys()):
            ...
            print(
                (
                    "complexity [%3d]:  %d * %d * %d log2( %d * %d) = %2.1f"
                    % (
                        idx,
                        data["di"],
                        data["ei"],
    ...
    def _calculate_score(self):
        ...
        for idx in list(self._indices.keys()):
            if not any(
                idx in double_bond for double_bond in self._cistrans_double_bond_atoms
            ):
                complexity = self._calculate_complexity(idx)
                self._indices[idx][
                    "complexity"
                ] = complexity  # complexity for a specific position
    ...
    def _find_cistrans_bond_atoms(self):
        ...
        for bond in self._find_potential_cistrans_bonds():
            ...
            )  # finds index of the first atom in the double bond
            db_atom_idx2 = (
                bond.GetEndAtom().GetIdx()
            )  # finds index of the second atom in the double bond
            double_bond_atoms = (
                self.mol.GetAtom(db_atom_idx1),
                self.mol.GetAtom(db_atom_idx2),
            )
            ...
            for index, db_atom in enumerate(double_bond_atoms):
                ...
                neighbours_atom_numbers_db = self._find_neighbours(
                    db_atom, return_atomic_nums=True
                )

                # checking if the atoms directly attached to one side of the double bond are different
                if (
                    len(set(neighbours_atom_numbers_db)) > 1
                    or len(neighbours_atom_numbers_db) == 1
                ):
                ...
                # if the atoms directly attached to the double bond are the same, further analysis of the attached groups is made
                    ...
                    branch1_atoms = [
                        neigh_atoms_db[0]
                    ]  # currently analysed atoms in the first group attached to one side of the double bond
                    branch2_atoms = [
                        neigh_atoms_db[1]
                    ]  # currently analysed atoms in the second group attached to the same side of the double bond
                    branches_equal = True
                    while branches_equal:
                        ...
                        branches_equal = self._branch_levels_are_equal(
                            branch1_atoms, branch2_atoms
                        )
                        # find neighbours of the currently analysed atoms in the branch/group
                        branch1_atoms = [
                            neigh
                            for atom in branch1_atoms
                            for neigh in self._find_neighbours(atom)
                        ]
                        branch2_atoms = [
                            neigh
                            for atom in branch2_atoms
                            for neigh in self._find_neighbours(atom)
                        ]
            ...
            if register_cistrans:
                self._cistrans_double_bond_atoms.append(
                    (db_atom_idx1, db_atom_idx2)
                )  # atom idxs are added as tuples
    ...
    def _branch_levels_are_equal(self, branch1_atoms, branch2_atoms):
        ...
        branch1_atomic_nums = sorted(
            [
                atom_num
                for atom in branch1_atoms
                for atom_num in self._find_neighbours(atom, return_atomic_nums=True)
            ]
        )
        branch2_atomic_nums = sorted(
            [
                atom_num
                for atom in branch2_atoms
                for atom_num in self._find_neighbours(atom, return_atomic_nums=True)
        ...
        branch1_bond_orders = sorted(
            [self._indices[atom.GetIdx()]["bi"] for atom in branch1_atoms]
        )
        branch2_bond_orders = sorted(
            [self._indices[atom.GetIdx()]["bi"] for atom in branch2_atoms]
        )

        # checks if the atomic numbers and bond orders are the same in both currently analysed atom branches
        if (
            branch1_atomic_nums == branch2_atomic_nums
            and branch1_bond_orders == branch2_bond_orders
        ):
    ...
    def _find_neighbours(self, atom, return_atomic_nums=False):
        ...
        for neigh in ob.OBAtomAtomIter(atom):
            if (
                neigh in self.excluded_atoms
            ):  # only searches for neighbours that are not in the excluded list
                continue
    ...
    def _calc_Vi(self, idx, atom):
        ...
        if self.debug:
            print("DEBUG> Vi[%3d]: v = %d | charge = %d" % (idx, valence, charge))
        vi = 8 - valence + charge
    ...
    def _calc_bi_ei_si(self, idx, atom):
        ...
        if (
            atom.IsChiral()
        ):  # does not take into consideration axial assymetry or cis-trans isomers  <---
            si += 1
        for neigh in ob.OBAtomAtomIter(atom):
...
def calculate_bottchscore_from_smiles(
    smiles: str,
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Tool to calculate Böttcher score on small molecules.",
        usage=None,
        epilog=epilog,
    ...
    parser.add_argument(
        "-m",
        action="store_true",
        help="disable mesomeric effect estimate",
        default=False,
    )
    parser.add_argument(
        "-p",
        action="store_true",
        help="generate PNG image of the structure",
        default=False,
    )
    parser.add_argument(
        "-c",
        action="store_true",
        help="add a progressive counter to the list of results shown",
        default=False,
    )
    parser.add_argument(
        "-v",
    ...
    parser.add_argument(
        ...
        "the default value is set to 3000000",
        metavar="MaxMemory",
        default=3000000,
    )
